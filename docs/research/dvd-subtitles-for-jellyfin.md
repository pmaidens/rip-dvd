...
When a client requires subtitle burn-in without server hardware acceleration,
the CPU must decode, overlay, and re-encode video. Include a long sample in
browser acceptance tests and inspect real-time transcode speed, not just
...
A corrected MKV may not appear until Jellyfin refreshes its library. Run Jellyfin's **Scan Media Library** task after publication and verify the refreshed stream list before judging subtitle playback.

...
Run this on the Compose host after the audit service has been deployed. The audit service opens SQLite read-only with `query_only`, mounts the originals library read-only, has no network, and emits path-free JSON. Keep the output in a restricted operator workspace because it contains archive and Detected Disc identifiers.

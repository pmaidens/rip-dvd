...
timeline. Errors such as `Invalid angle block`, `chapter NOT FOUND!`, or
`dvdnav_sector_search failed` can leave a truncated output that still passes
stream and bounded-decode checks.
...
libdvdread reads title content without navigating menus. The worker does not first

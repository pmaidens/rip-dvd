...
When ffprobe identifies a track but omits `nb_read_packets` after a complete
packet-count probe, cleanup removes the empty track. Validation must also
exclude that track from the source expectations to avoid rejecting the
cleaned output for a subtitle count mismatch.

...
- Validate the existing dark dashboard design at desktop and narrow mobile widths.
...
The supported deployment uses a coordinated web/worker/data-access version over one SQLite file. A single reviewed rebuild migration is acceptable. Before deployment, use the existing backup workflow; recovery should restore the pre-migration database and prior coordinated application version. Prefer roll-forward once new writes have begun.
...
- `sh scripts/smoke-compose-workers.sh all` — passed the native reader compile/test and named-volume plus bind-mount archive/encode worker smoke paths under UID 1000 after final review remediation.
...
6. On completion, record validation results, any deliberate deviations, migration recovery notes, and links to synthetic test screenshots in this plan before marking it `complete`.

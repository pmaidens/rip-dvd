...
- Dependencies: P1-T01
- Blocks: none
- Deferred reason: revisit when profiling shows that catalog history makes the exact 30-second summary too costly.

## Why

The exact action overview currently reads complete matching histories inside one SQLite snapshot, then counts and sorts them in application memory. Its cost grows with permanent archive and job history.

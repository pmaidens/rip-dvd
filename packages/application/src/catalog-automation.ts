...
import { formatVolumeLabel } from "./catalog-label.js";
...
export async function suggestCatalog(
  ...
  const withCandidates = (suggestion: AutomaticCatalogSuggestion): AutomaticCatalogSuggestion => ({
    ...suggestion,
    candidates,
  });
  ...
  if (metadataSelection !== undefined && operatorSelectedCandidate === undefined) {
    return withCandidates(needsReview(
      hints,
      "no_metadata_match",
  ...
  if (candidate.kind === "movie") {
    return withCandidates({ status: "ready", hints, proposal: movieProposal(candidate, titles) });
  }
  ...
  return withCandidates(proposal === null
    ? needsReview(
      hints,
      "episode_order_uncertain",

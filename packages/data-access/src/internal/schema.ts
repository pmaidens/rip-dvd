...
export const originalDiscArchives = sqliteTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(${table.integrity} = 'unknown' and ${table.integrityPolicyVersion} is null and ${table.badSectorCount} is null and ${table.badAreaCount} is null and ${table.badSectorRanges} is null) or (${table.integrity} = 'clean_read' and length(${table.integrityPolicyVersion}) between 1 and 128 and ${table.badSectorCount} = 0 and ${table.badAreaCount} = 0 and json(${table.badSectorRanges}) = json('[]')) or (${table.integrity} = 'watchable_salvage' and length(${table.integrityPolicyVersion}) between 1 and 128 and ${table.badSectorCount} > 0 and ${table.badAreaCount} > 0 and json_valid(${table.badSectorRanges}) and json_type(${table.badSectorRanges}) = 'array')`,
    ),
    check(

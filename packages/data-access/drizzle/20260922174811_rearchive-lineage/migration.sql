ALTER TABLE `archive_requests` ADD `rearchive_source_archive_id` text REFERENCES original_disc_archives(id) ON DELETE RESTRICT;--> statement-breakpoint
ALTER TABLE `original_disc_archives` ADD `rearchive_source_archive_id` text REFERENCES original_disc_archives(id) ON DELETE RESTRICT;--> statement-breakpoint
...
DROP INDEX IF EXISTS `original_disc_archives_detected_disc_unique`;--> statement-breakpoint
DROP INDEX IF EXISTS `original_disc_archives_fingerprint_unique`;--> statement-breakpoint
...
CREATE INDEX `archive_requests_rearchive_source_idx` ON `archive_requests` (`rearchive_source_archive_id`,`created_at`,`id`);
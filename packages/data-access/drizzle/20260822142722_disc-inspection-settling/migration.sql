ALTER TABLE `disc_inspections` ADD `media_capacity_bytes` integer;--> statement-breakpoint
ALTER TABLE `disc_inspections` ADD `stable_observation_count` integer;--> statement-breakpoint
ALTER TABLE `disc_inspections` ADD `settling_quiet_window_started_at` integer;--> statement-breakpoint
ALTER TABLE `disc_inspections` ADD `settling_started_at` integer;--> statement-breakpoint
ALTER TABLE `disc_inspections` ADD `settling_reset_count` integer;--> statement-breakpoint
...
CREATE TABLE `__new_disc_inspections` (
	...
	CONSTRAINT "disc_inspections_settling_evidence_check" CHECK((("media_capacity_bytes" is null and "stable_observation_count" is null and "settling_quiet_window_started_at" is null and "settling_started_at" is null and "settling_reset_count" is null) or (typeof("media_capacity_bytes") = 'integer' and "media_capacity_bytes" > 0 and "media_capacity_bytes" <= 9000000000 and "media_capacity_bytes" % 2048 = 0 and typeof("stable_observation_count") = 'integer' and "stable_observation_count" between 1 and 3 and "settling_quiet_window_started_at" is not null and "settling_started_at" is not null and "settling_quiet_window_started_at" >= "settling_started_at" and typeof("settling_reset_count") = 'integer' and "settling_reset_count" between 0 and 10000)) and ("phase" <> 'settling' or "media_capacity_bytes" is not null)),
...
CREATE INDEX `disc_inspections_status_idx` ON `disc_inspections` (`status`,`updated_at`);--> statement-breakpoint
